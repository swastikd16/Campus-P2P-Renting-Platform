...
const SYSTEM_INSTRUCTION = `You are an intelligent assistant for "Campus P2P Renting by CipherOps", a student renting platform in India. 
...
export const suggestDescription = async (title: string, category: string, condition: string): Promise<string> => {
  try {
    const response = await ai.models.generateContent({
      ...
      Category: ${category}
      Condition: ${condition}
      Target Audience: Indian University Students.`,
      config: {
...
export const suggestPrice = async (title: string, originalPrice: number): Promise<{ price: number; reasoning: string }> => {
  try {
    const response = await ai.models.generateContent({
      ...
      The context is a student campus (peer-to-peer). Price should be affordable but fair.
      Return JSON only.`,
      config: {
        systemInstruction: SYSTEM_INSTRUCTION,
...
export const analyzeSafety = async (item: Item): Promise<string[]> => {
   try {
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Give 3 short safety tips for renting out or borrowing a "${item.title}" (${item.category}) in a college campus context.
